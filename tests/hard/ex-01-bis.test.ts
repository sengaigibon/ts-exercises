import { List} from "../../exercises/hard/exercise-01-bis";
...
describe('Implement basic list operations', () => {
    ...
    test('reverse: given a list, return a list with all the original items, but in reversed order', () => {
        ...
        expect(reversed.getValues()).toStrictEqual([886, 400, 100, 90]);
    });
    ...
    test('filter: given a predicate and a list, return the list of all items for which predicate(item) is True', () => {
        ...
        let filtered = list.filter<number>(num => num > 100);
        expect(filtered.getValues()).toStrictEqual([400, 886, 124])

        filtered = list.filter<number>(num => num < 100);
        expect(filtered.getValues()).toStrictEqual([90, 8, 14]);
    });

    test('map: given a function and a list, return the list of the results of applying function(item) on all items', () => {
        const list = List.create(90, 100, 400, 886);
        let modified = list.map<number>(num => num / 2);
        expect(modified.getValues()).toStrictEqual([45, 50, 200, 443]);
        ...
        expect(modified.getValues()).toStrictEqual([180, 200, 200, 443]);
    });
    ...
    test('foldl: given a function, a list, and initial accumulator, fold (reduce) each item into the accumulator from the left', () => {
        ...
        let result = list.foldl<number>((acc, num) => acc + num, 0);
        expect(result).toBe(10);

        list = List.create(1, 2, 3, -4);
        result = list.foldl<number>((acc, num) => acc - num, 0);
